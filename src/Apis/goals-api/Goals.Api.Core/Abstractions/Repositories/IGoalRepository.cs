...
public interface IGoalRepository
{
    ...
    Task<Goal> CreateAsync(Goal goal, bool saveChanges = true, CancellationToken token = default);
    Task<IReadOnlyList<Goal>> SearchAsync(Expression<Func<Goal, bool>> options = null, CancellationToken token = default);
    Task<Goal> UpdateAsync(Goal goal, bool saveChanges = true, CancellationToken token = default);
}
