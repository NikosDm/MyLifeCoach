...
public interface IGoalTypeRepository
{
    ...
    Task<GoalType> CreateAsync(GoalType entity, bool saveChanges = true, CancellationToken token = default);
    Task<GoalType> UpdateAsync(GoalType entity, bool saveChanges = true, CancellationToken token = default);
}
